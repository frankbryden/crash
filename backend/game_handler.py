...
class Game:
    def __init__(self,name : str,starting_multiplicator : float = 0.75,multiplicator_coef : float = 0.01,
                 average : float = 30,standard_deviation : float = 10, minimum_time : float = 3,
                 players : list = None):
        self.name = name
    ...
    def start_game(self):
        if not self.ongoing: 
            self.game_duration = max(self.min_time,np.random.normal(self.average,self.std))
            self.ongoing = True
...
class GameHandler:
    ...
    def get_game(self,name : str = "New Game")-> Game:
        ...
        return Game(name,
                    ...
                    average = self.average,standard_deviation = self.std,
